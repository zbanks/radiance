...
impl<'a> VideoNodeTile<'a> {
   pub fn new(id_source: impl Hash, rect: Rect, inputs: &'a [f32], outputs: &'a [f32]) -> Self {
    ...
    fn paint(&self, ui: &Ui, response: &Response) {
        ...
        vertices.push(Vector2::<f32>::new(self.rect.left(), self.rect.top()));
        ...
        for (&loc, &size) in self.inputs.iter().zip(input_sizes.iter()) {
            ...
            if size > 0. {
                vertices.push(Vector2::<f32>::new(self.rect.left(), self.rect.top() + loc - size));
                vertices.push(Vector2::<f32>::new(self.rect.left() + size, self.rect.top() + loc));
                vertices.push(Vector2::<f32>::new(self.rect.left(), self.rect.top() + loc + size));
            }
            last_loc = loc;
        }
        vertices.push(Vector2::<f32>::new(self.rect.left(), self.rect.bottom()));
        vertices.push(Vector2::<f32>::new(self.rect.right(), self.rect.bottom()));
        ...
        for (&loc, &size) in self.outputs.iter().rev().zip(output_sizes.iter().rev()) {
            ...
            if size > 0. {
                vertices.push(Vector2::<f32>::new(self.rect.right(), self.rect.top() + loc + size));
                vertices.push(Vector2::<f32>::new(self.rect.right() + size, self.rect.top() + loc));
                vertices.push(Vector2::<f32>::new(self.rect.right(), self.rect.top() + loc - size));
            }
            last_loc = loc;
        }
        vertices.push(Vector2::<f32>::new(self.rect.right(), self.rect.top()));
        ...
        for pos in vertices.iter() {
            mesh.colored_vertex(pos2(pos.x, pos.y), Color32::BLACK);
            points.push(pos2(pos.x, pos.y));
        }
